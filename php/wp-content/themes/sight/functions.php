...
$meta_box = array(
    'id' => 'slide',
    'title' => 'Настройки слайдшоу',
    'page' => 'post',
    'context' => 'side',
    'priority' => 'low',
    'fields' => array(
        array(
            'name' => 'Показывать в слайдшоу',
            'id' => $prefix . 'slide',
            'type' => 'checkbox'
        )
    )
...
function options_admin_menu() {
	...
	add_theme_page("Настройки темы Sight", "Настройки темы", 'edit_themes', basename(__FILE__), 'options_page');
}
    ...
    <div class="wrap">
        ...
        <form method="post" action="">
            ...
            <table class="form-table">
                <tr valign="top">
                    <th scope="row"><label for="logo_url"><?php _e('Путь к логотипу:'); ?></label></th>
                    <td><input type="text" name="logo_url" id="logo_url" size="50" value="<?php echo get_option('logo_url'); ?>"/><br/><span
                            class="description"> <a href="<?php bloginfo("url"); ?>/wp-admin/media-new.php" target="_blank">Загрузите логотип</a> (максимум 290px x 128px) через медиабиблиотеку WordPress, скопируйте и вставьте ссылку здесь</span><br/><br/><img src="<?php echo (get_option('logo_url')) ? get_option('logo_url') : get_bloginfo('template_url') . '/images/logo.png' ?>"
                     alt=""/></td>
                </tr>
                <tr valign="top">
                    <th scope="row"><label for="bg_color"><?php _e('Цвет фона:'); ?></label></th>
                    <td><input type="text" name="bg_color" id="bg_color" size="20" value="<?php echo get_option('bg_color'); ?>"/><span
                            class="description">например, <strong>#27292a</strong> или <strong>black</strong></span></td>
                </tr>
                <tr valign="top">
                    <th scope="row"><label for="ss_disable"><?php _e('Отключить слайдшоу:'); ?></label></th>
                    <td><input type="checkbox" name="ss_disable" id="ss_disable" <?php echo (get_option('ss_disable'))? 'checked="checked"' : ''; ?>/></td>
                </tr>
                <tr valign="top">
                    <th scope="row"><label for="ss_timeout"><?php _e('Таймаут для слайдшоу (в миллисекундах):'); ?></label></th>
                    <td><input type="text" name="ss_timeout" id="ss_timeout" size="20" value="<?php echo get_option('ss_timeout'); ?>"/><span
                            class="description">например, <strong>7000</strong></span></td>
                </tr>
                <tr valign="top">
                    <th scope="row"><label><?php _e('Пагинация:'); ?></label></th>
                    <td>
                        <input type="radio" name="paging_mode" value="default" <?php echo (get_option('paging_mode') == 'default')? 'checked="checked"' : ''; ?>/><span class="description">По-умолчанию + поддежка WP Page-Navi</span><br/>
                        <input type="radio" name="paging_mode" value="ajax" <?php echo (get_option('paging_mode') == 'ajax')? 'checked="checked"' : ''; ?>/><span class="description">AJAX при загрузке записей</span><br/>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row"><label for="ga"><?php _e('Код Google Analytics:'); ?></label></th>
                    <td><textarea name="ga" id="ga" cols="48" rows="18"><?php echo get_option('ga'); ?></textarea></td>
                </tr>
            </table>

            <p><input type="submit" value="Сохранить изменения" class="button button-primary" /></p>
        </form>
    </div>
<?php
}
...
class GetConnected extends WP_Widget {
        ...
        ?>
            <?php echo $before_widget; ?>
                <?php if ( $title )
                    ...
                    <a href="<?php echo ( get_option('feedburner_url') )? get_option('feedburner_url') : get_bloginfo('rss2_url'); ?>">RSS-лента</a>
                    ...
                    <span><?php if ( function_exists('twittercount') ) twittercount( get_option('twitter_url') ); ?> читателей</span>
                    </div>
                    <?php endif; ?>
                    <!-- /Twitter -->
                    ...
                    <a href="<?php echo get_option('flickr_url'); ?>">Группа Flickr</a>
    ...
    function form($instance) {
        ...
        ?>
            <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Заголовок:'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label></p>
            ...
            <div style="margin-bottom: 5px;">
                ...
                <p class="social_options">
                    <label for="feedburner_url"><?php _e('Ссылка в FeedBurner:'); ?></label>
            ...
            <div style="margin-bottom: 5px;">
                ...
                <p class="social_options">
                    <label for="twitter_url">Ссылка на профиль:</label>
...
class Recentposts_thumbnail extends WP_Widget {
        ...
        ?>
            ...
            ?>
                ...
                <a href="<?php the_permalink(); ?>" class="rpthumb clear">
                    ...
                    <span class="rpthumb-date" <?php echo $offset; unset($offset); ?>><?php the_time(__('d M Y')) ?></span>
                </a>
    ...
    function form($instance) {
        ...
        ?>
            <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Заголовок:'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label></p>
            <p><label for="rpthumb_qty">Количество записей:  </label><input type="text" name="rpthumb_qty" id="rpthumb_qty" size="2" value="<?php echo get_option('rpthumb_qty'); ?>"/></p>
            <p><label for="rpthumb_thumb">Прятать миниатюры:  </label><input type="checkbox" name="rpthumb_thumb" id="rpthumb_thumb" <?php echo (get_option('rpthumb_thumb'))? 'checked="checked"' : ''; ?>/></p>
        <?php
    }
                ...
                <tr>
                    ...
                    <td>
                        <div class="comment-meta">
                            <?php printf(__('<p class="comment-author"><span>%s</span> пишет:</p>'), get_comment_author_link()) ?>
                            <?php printf(__('<p class="comment-date">%s</p>'), get_comment_date('d M Y')) ?>
                    ...
                    <td>
                        <div class="comment-text">
                            <?php if ($comment->comment_approved == '0') : ?>
                                <p><?php _e('Спасибо! Ваш комментарий ожидает проверки.') ?></p>
                                <br/>
                            <?php endif; ?>
                            <?php comment_text() ?>
                        </div>
...
function feedcount($feedurl='http://feeds.feedburner.com/wpshower') {
    ...
    if ( $bis != 'fail' && $burner_count_circulation != '' ) {
        echo '<span>'.$burner_count_circulation.' читателей</span>';
    } else {
