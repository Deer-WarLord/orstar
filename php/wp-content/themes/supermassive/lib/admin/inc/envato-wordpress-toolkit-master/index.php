...
class Envato_WP_Toolkit {
  ...
  protected function _constants() {
     ...
     * Plugin Name
     */
    define( 'EWPT_PLUGIN_NAME', __( 'Envato WordPress Toolkit', 'gp_lang' ) );
     ...
     */
    define( 'EWPT_PLUGIN_DIR', gp_admin . 'envato-wordpress-toolkit-master/' ); // MODIFIED
    
    /**
     * Plugin Directory URL
     */
    define( 'EWPT_PLUGIN_URL', get_template_directory_uri() . '/lib/admin/inc/envato-wordpress-toolkit-master/' ); // MODIFIED
  ...
  public function _envato_menu() {
    $menu_page = add_menu_page( EWPT_PLUGIN_NAME, __( 'Envato Toolkit', 'gp_lang' ), 'manage_options', EWPT_PLUGIN_SLUG, array( &$this, '_envato_menu_page' ), EWPT_PLUGIN_URL . 'assets/images/envato.png', 59 );
  ...
  public function _envato_menu_page() {
    if ( ! current_user_can( 'manage_options' ) )
      wp_die( __( 'You do not have sufficient permissions to access this page.', 'gp_lang' ) );
    ...
    if ( empty( $errors ) ) {
      echo ( isset( $_GET[ 'settings-updated' ] ) ) ? '<div class="updated below-h2"><p><strong>' . __( 'User Settings Updated.', 'gp_lang' ) . '</strong></p></div>' : '';
      echo ( isset( $_GET[ 'activated' ] ) ) ? '<div class="updated below-h2"><p><strong>' . __( 'Theme Activated.', 'gp_lang' ) . '</strong></p></div>' : '';
      echo ( isset( $_GET[ 'deleted' ] ) ) ? '<div class="updated below-h2"><p><strong>' . __( 'Theme Deleted.', 'gp_lang' ) . '</strong></p></div>' : '';
    }
    ...
    /* display normal views */
      ...
      echo '<div class="wrap">';
        ...
        <form name="verification_form" method="post" action="' . admin_url( 'options.php' ) . '" id="api-verification">';
          ...
          echo '
          <p class="submit">
            <input type="submit" name="Submit" class="button-primary right" value="' . __( 'Save Settings', 'gp_lang' ) . '" />
          </p>
        </form>';
        ...
        if ( empty( $errors ) && count( $themes ) > 0 ) {
          ...
          foreach( $themes as $theme ) {
            ...
            $delete_onclick = 'onclick="if ( confirm(\'' . esc_js( sprintf( __( "You're about to delete the '%s' theme. 'Cancel' to stop, 'OK' to update.", 'gp_lang' ), $title ) ) . '\') ) {return true;}return false;"';
            ...
            $update_onclick = 'onclick="if ( confirm(\'' . esc_js( __( "Updating this theme will lose any customizations you have made. 'Cancel' to stop, 'OK' to update.", 'gp_lang' ) ) . '\') ) {return true;}return false;"';
            ...
            if ( $stylesheet && $template && $current_stylesheet !== $stylesheet ) {
              $links[] = '<a href="' . $activate_url .  '" class="activatelink" title="' . esc_attr( sprintf( __( 'Activate &#8220;%s&#8221;', 'gp_lang' ), $title ) ) . '">' . __( 'Activate', 'gp_lang' ) . '</a>';
              $links[] = '<a href="' . $preview_url . '" class="thickbox thickbox-preview" title="' . esc_attr( sprintf( __( 'Preview &#8220;%s&#8221;', 'gp_lang' ), $title ) ) . '">' . __( 'Preview', 'gp_lang' ) . '</a>';
              $links[] = '<a href="' . $delete_url . '" class="submitdelete deletion" title="' . esc_attr( sprintf( __( 'Delete &#8220;%s&#8221;', 'gp_lang' ), $title ) ) . '" ' . $delete_onclick . '>' . __( 'Delete' ) . '</a>';
              $links[] = '<a href="' . $details_url . '" class="thickbox thickbox-preview" title="' . esc_attr( sprintf( __( 'View version %1$s details', 'gp_lang' ), $latest_version ) ) . '">' . esc_attr( sprintf( __( 'View version %1$s details', 'gp_lang' ), $latest_version ) ) . '</a>';
              if ( ! empty( $theme_backup_uri ) ) {
                $links[] = '<a href="' . $theme_backup_uri . '" title="' . esc_attr( __( 'Download Backup', 'gp_lang' ) ) . '">' . esc_attr( __( 'Download Backup', 'gp_lang' ) ) . '</a>';
              }
            ...
            if ( $current_stylesheet == $stylesheet ) {
              ...
              if ( ! empty( $theme_backup_uri ) ) {
                $options[] = '<a href="' . $theme_backup_uri . '" title="' . esc_attr( __( 'Download Backup', 'gp_lang' ) ) . '">' . esc_attr( __( 'Download Backup', 'gp_lang' ) ) . '</a>';
              }
              if ( ! empty( $options ) )
                $content.= '<div class="update-info"><span>' . __( 'Options:', 'gp_lang' ) . '</span> ' . implode( ' | ', $options ) . '</div>';
            }
            
            /* Theme path information */
            if ( current_user_can( 'edit_themes' ) && $installed ) {
              if ( $parent_theme ) {
                 $content.= '<p>' . sprintf( __( 'The template files are located in <code>%2$s</code>. The stylesheet files are located in <code>%3$s</code>. <strong>%4$s</strong> uses templates from <strong>%5$s</strong>. Changes made to the templates will affect both themes.', 'gp_lang' ), $title, str_replace( WP_CONTENT_DIR, '', $template_dir ), str_replace( WP_CONTENT_DIR, '', $stylesheet_dir ), $title, $parent_theme ) . '</p>';
              } else {
                 $content.= '<p>' . sprintf( __( 'All of this theme&#8217;s files are located in <code>%2$s</code>.', 'gp_lang' ), $title, str_replace( WP_CONTENT_DIR, '', $template_dir ), str_replace( WP_CONTENT_DIR, '', $stylesheet_dir ) ) . '</p>';
              }
            }
            ...
            if ( $has_update ) {
              if ( ! current_user_can( 'update_themes' ) ) {
                $content.= sprintf( '<div class="updated below-h2"><p><strong>' . __( 'There is a new version of %1$s available. <a href="%2$s" class="thickbox thickbox-preview" title="%1$s">View version %3$s details</a>.', 'gp_lang' ) . '</strong></p></div>', $title, $details_url, $latest_version );
              } else {
                $content.= sprintf( '<div class="updated below-h2"><p><strong>' . __( 'There is a new version of %1$s available. <a href="%2$s" class="thickbox thickbox-preview" title="%1$s">View version %3$s details</a> or <a href="%4$s" %5$s>update automatically</a>.', 'gp_lang' ) . '</strong></p></div>', $title, $details_url, $latest_version, $update_url, $update_onclick );
              }
            } else if ( ! $installed ) {
              if ( ! current_user_can( 'update_themes' ) ) {
                $content.= sprintf( '<div class="updated below-h2"><p><strong>' . __( '%1$s has not been installed. <a href="%2$s" class="thickbox thickbox-preview" title="%1$s">View version %3$s details</a>.', 'gp_lang' ) . '</strong></p></div>', $title, $details_url, $latest_version );
              } else {
                $content.= sprintf( '<div class="updated below-h2"><p><strong>' . __( '%1$s has not been installed. <a href="%2$s" class="thickbox thickbox-preview" title="%1$s">View version %3$s details</a> or <a href="%4$s">install automatically</a>.', 'gp_lang' ) . '</strong></p></div>', $title, $details_url, $latest_version, $install_url );
              }
            }
  ...
  public function _admin_init() {
    ...
    add_settings_section( 'user_account_info', __( 'User Account Information', 'gp_lang' ), array( &$this, '_section_user_account_info' ), EWPT_PLUGIN_SLUG );
    add_settings_field( 'user_name', __( 'Marketplace Username', 'gp_lang' ), array( &$this, '_field_user_name' ), EWPT_PLUGIN_SLUG, 'user_account_info' );
    add_settings_field( 'api_key', __( 'Secret API Key', 'gp_lang' ), array( &$this, '_field_api_key' ), EWPT_PLUGIN_SLUG, 'user_account_info' );
    add_settings_section( 'backup_info', __( 'Backup Information', 'gp_lang' ), array( &$this, '_section_backup_information' ), EWPT_PLUGIN_SLUG );
    add_settings_field( 'skip_theme_backup', __( 'Skip Theme Backup', 'gp_lang' ), array( &$this, '_field_skip_theme_backup' ), EWPT_PLUGIN_SLUG, 'backup_info' );
  }
  ...
  public function _section_user_account_info() {
    _e( 'To obtain your API Key, visit your "My Settings" page on any of the Envato Marketplaces. Once a valid connection has been made any changes to the API key below for this username will not effect the results for 5 minutes because they\'re cached in the database. If you have already made an API connection and just purchase a theme and it\'s not showing up, wait five minutes and refresh the page. If the theme is still not showing up, it\'s possible the author has not made it available for auto install yet.', 'gp_lang' );
  }
    ...
    if ( isset( $_GET['page'] ) && isset( $_GET['action'] ) ) {
      ...
      if ( $page == EWPT_PLUGIN_SLUG ) {
        if ( 'install-theme' == $action || 'upgrade-theme' == $action ) {
          $actions['themes_page'] = '<a href="' . network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG ) . '" title="' . esc_attr__( sprintf( __( 'Return to %s', 'gp_lang' ), EWPT_PLUGIN_NAME ) ) . '" target="_parent">' . sprintf( __( 'Return to %s', 'gp_lang' ), EWPT_PLUGIN_NAME ) . '</a>';
        }
      }
    }
    return $actions;
  ...
  protected function _install_theme( $theme, $themes ) {
    ...
    if ( ! current_user_can( 'install_themes' ) )
      wp_die( __( 'You do not have sufficient permissions to install themes for this site.', 'gp_lang' ) );
    ...
    $title = sprintf( __( 'Installing Theme: %s', 'gp_lang' ), $api->name . ' ' . $api->version );
  ...
  protected function _activate_theme( $template, $stylesheet ) {
    ...
    if ( ! current_user_can( 'switch_themes' ) && ! current_user_can( 'edit_theme_options' ) )
      wp_die( __( 'You do not have sufficient permissions to update themes for this site.', 'gp_lang' ) );
  ...
  protected function _delete_theme( $template ) {
    ...
    if ( ! current_user_can( 'delete_themes' ) )
      wp_die( __( 'You do not have sufficient permissions to delete themes for this site.', 'gp_lang' ) );
  ...
  function _prepare_envato_backup() {
    ...
    $contents[]	= '# ' . __( 'This .htaccess file ensures that other people cannot download your backup files.', 'gp_lang' );
    $contents[] = '';
