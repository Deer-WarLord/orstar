...
function if_presstrends_theme_options() {
	if (!current_user_can('manage_options'))  {
		wp_die( __('У вас нет достаточно прав для доступа к этой странице.') );
	}
?>
	<form action="options.php" method="post">
	    ...
	    <p class="submit">
	        <input name="Submit" type="submit" value="<?php esc_attr_e('Обновить'); ?>" />
	    </p>
	</form>
<?php
}
...
function if_presstrends_theme_init(){
	...
	add_settings_field('presstrends_opt_in', 'Активировать PressTrends?', 'if_presstrends_opt_string', 'presstrends_top', 'presstrends_top');
}

// PressTrends Section Text
function if_presstrends_top_text() {
    echo '<p style="width:190px;float:left;"><img src="http://presstrends.io/images/logo.png"/></p><p style="width:500px;float:left;color:#777;padding-top:2px;"><b>PressTrends</b> помогает разработчикам делать темы лучше, благодаря сбору статистической информации. PressTrends также предоставляет <a href="http://wordpress.org/extend/plugins/presstrends/" title="PressTrends Plugin for WordPress" target="_blank">плагин</a>, который предоставляет информацию о том, как функционирует ваш сайт в сети, и по сравнению с другими такими же сайтами. <a href="http://presstrends.io" title="PressTrends" target="_blank">Learn more…</a></p>';
}
...
function if_be_password_pointer_print_admin_bar() {
	$pointer_content  = '<h3>' . 'Активировать PressTrends' . '</h3>';
	$pointer_content .= '<p>' . 'Помогите разработчикам делать темы лучше, благодаря сбору статистической информации' . '</p>';
